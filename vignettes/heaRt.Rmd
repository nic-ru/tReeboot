...
The _heaRt_ package is designed for users who want to explore heart disease
data without dealing with complex preprocessing or model specification.

The _heaRt_ package deliberately restricts the choice of predictors to
predefined pairs of variables. This choice ensures that all models are fitted on clinically meaningful combinations of variables, that the user is not required to specify model formulas and
that results remain easy to interpret and compare.
...
dat <- load_heaRt(vars = "rbp-restECG", severe_diag = FALSE)
head(dat)
```
...
For logistic regression, the fitted curve represents the estimated
probability of heart disease.

#### Two predictors
When two predictors are used, the data are displayed in the
two-dimensional predictor space, with the fitted relationship overlaid.
...
All regression-based plots are produced using ggplot2, ensuring
consistent aesthetics and easy extensibility.
