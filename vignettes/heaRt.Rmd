...
When the number of predictors is equal to two, the logical argument `consider_first` determines whether the first or the second predictor should be used in single predictor models.
...
-   random forests, for non-linear classification based on groups of decision trees;
...
Additional classes corresponding to the underlying model (e.g. `"rpart"` or `"randomForest"`) are attached when relevant. This class structure is central to the design of the package, as it allows the `plot()` method to automatically adapt its behavior to the type of fitted model without requiring additional input from the user.
...
# Logistic regression
mod_log  <- fit(dat, num_var = "2", fit_type = "logistic.reg", 
                consider_first = FALSE)
```

#### Common mistakes avoided by heaRt

Normally, manual formula specification is required for examples like:
`glm(y ~ rbp + restECG, family = binomial)`.

With heaRt, the structure is applied automatically:
...
For linear and logistic regression models, the type of plot depends on the number of predictors.
...
# Linear model
mod_lm2  <- fit(dat, "2", "lm")

# Logistic regression
mod_log2 <- fit(dat, "2", "logistic.reg")

# Decision tree
mod_tree2 <- fit(dat, "2", "decision.tree")

# Random forest
mod_rf2  <- fit(dat, "2", "random.forest")
```
